...
namespace PnnLABQuant
{
	...
	bool PnnLABQuantizer::QuantizeImage(Bitmap* pSource, Bitmap* pDest, UINT& nMaxColors, bool dither)
	{
		...
		if (nMaxColors <= 32 || (semiTransCount * 1.0 / pixels.size()) > .099)
			Peano::GilbertCurve::dither(bitmapWidth, bitmapHeight, pixels.data(), pPalette, closestColorIndex, GetColorIndex, qPixels.get(), 1.0f);
		else {
