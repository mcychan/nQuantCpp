	...
	for (int i = 0; i < height; i++) {
		...
		for (j = 0; j < width; j++) {
			...
			if (c2.GetA() < BYTE_MAX && c.GetA() == BYTE_MAX) {
				lookup[offset] = ditherFn(pPalette, nMaxColors, pixels[pixelIndex]) + 1;
				qPixels[pixelIndex] = lookup[offset] - 1;
...
bool dither_image(const ARGB* pixels, DitherFn ditherFn, bool& hasSemiTransparency, const int& transparentPixelIndex, short* qPixels, UINT width, UINT height)
{
	UINT nMaxColors = 65536;
