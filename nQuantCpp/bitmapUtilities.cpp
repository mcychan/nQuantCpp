	...
	const UINT width, const UINT height, const vector<float>& saliencies, bool enforcedDither, unsigned int frameIndex)
{
	...
	for (UINT y = 0; y < height; ++y)
	{
		for (UINT x = 0; x < width; ++x)
		{
			...
			auto noisyArgb = dither_pixel(pixels, pixelIndex, width, noiseDampener, baseSpread,
...
bool ProcessImagePixels(Bitmap* pDest, const unsigned short* qPixels, const bool hasTransparent)
{
	...
	for (UINT y = 0; y < h; ++y) {	// For each row...
		for (UINT x = 0; x < w; ++x) {	// ...for each pixel...
			BYTE nibbles = 0;
			auto index = static_cast<BYTE>(qPixels[pixelIndex++]);
...
bool GrabPixels(Bitmap* pSource, vector<ARGB>& pixels, int& semiTransCount, int& transparentPixelIndex, ARGB& transparentColor, const BYTE alphaThreshold, const UINT nMaxColors)
{
	...
	auto paletteSize = pSource->GetPaletteSize();
